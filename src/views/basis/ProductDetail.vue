	...
	<div>
			...
			<van-tabs v-model="activeTabName" stretch @click="handleClick">
					...
					<div>
						...
						<div class="groupbuy-item" v-if="detaildata.ifpd=='0'">
							...
							<div class="groupbuy-item-info">
								If a Group Buy consists of less than 2 buyers, a refund will be automatically
						...
						<div class="buyer-protection">
							<div class="buyer-protection1">
								...
								<div class="buyer-protection-info">
									<p>
										<van-icon name="certificate" /><span><em>Full Refund</em> if you don't receive
											your order.</span>
									</p>
									<p>
										<van-icon name="certificate" /><span><em>Full or Partial Refund</em> if the item
											is not as described.</span>
									</p>
								</div>
							</div>
						</div>
						...
						<div class="detail">
							<div class="head">
								...
								<div class="right" @click="_handShowDetail">View</div>
							</div>
							<div class="desc" v-show="showDetailInfo" v-html="detailInfo"></div>
						</div>
				...
				<van-tab title="Store" name="Store">
					<div class="detail-store" ref="storeInfo" id="storeInfo">
						<router-link :to="'/shop?id='+detaildata.pstoreid" style="display: block;">
					...
					<div v-if="isstore">
						<div id='parentId'>
							<ProductList :productList="shopProductList" :replace="true"
								@loadproductdata="loadproductdata" :productTotal="shopProductTotal" />
						</div>
					</div>
				</van-tab>
			</van-tabs>
		...
		<van-sku v-model="show" :sku="sku" :goods-id="pid" :goods="goods" :reset-selected-sku-on-hide="true"
			...
			<template #sku-actions="props">
				<div class="van-sku-actions">
					<van-button v-if="isBtn==0" square size="large" type="warning"
						@click="props.skuEventBus.$emit('sku:buy')">Add Cart</van-button>
					<van-button v-else-if="isBtn==1" square size="large" type="warning"
						@click="props.skuEventBus.$emit('sku:buy')">Buy Now</van-button>
					<!-- 直接触发 sku 内部事件，通过内部事件执行 onBuyClicked 回调 -->
					<van-button v-else-if="isBtn==3" square size="large" type="danger"
						@click="props.skuEventBus.$emit('sku:buy')">Share Get</van-button>
					<van-button v-else square size="large" type="danger" @click="props.skuEventBus.$emit('sku:buy')">
						Group Shopping</van-button>
				</div>
			</template>
		</van-sku>
		...
		<van-popup v-model="helpShow" round position="bottom">
			<div class="help-show">
				...
				<p>3. We will refund to you after we confirm the order status and you can use the balance to purchase
					products online or withdraw cash.</p>
			</div>
		</van-popup>
	</div>
</template>
...
<script>
	...
	export default ({
		...
		methods: {
			...
			hankOpen() {
				this.$router.push({
					path: `/shop/${this.detaildata.pstoreid}`
				});
			},
			//获取商品信息
			async getSCProductInfo() {
				this.loading.isload = true
				await this.$api.product.getSCProductInfo({
					pid: this.pid
				}).then((res) => {
			...
			async getNewsContent() {
				const res = await this.$api.product.getNewsContent({
					newid: this.detaildata.pid
				});
				if (res.rescode == 0) {
					let txt = res.fsmes;
					txt = txt.replace(/~\|~/g, '[').replace(/~\|\|~/g, ']').replace(/~\^~/g, '{').replace(
							/~\^\^~/g, '}').replace(/~rn~/g, '\r\n').replace(/~n~/g, '\n').replace(/~r~/g, '\r')
							.replace(/~t~/g, '\t').replace(/~\\~/g, '\\').replace(/800px/g,'100vw').replace(/\<img/g,'<img style="width:100vw"');
					this.detailInfo = txt;
				}
			},
			//获取是否关注商品
			async getLike() {
				await this.$api.product.getSaleFocus({
					pid: this.pid
				}).then((res) => {
					//console.log(res)
					if (res == 1) {
			...
			handleClick(name, title) {
				if (name == 'Store') {
					...
					if (this.shopProductList.length < 1) {
						...
						setTimeout(() => {
							this.scrollView = document.documentElement.clientHeight - document.getElementById(
								'storeInfo').offsetHeight - 105;
						}, 100)
					}
				}
			},
					...
					attrTitle.forEach(title => { //遍历sku熟悉属性
						...
						skuattr.forEach(item => { //遍历整个sku信息
							...
							item.v.forEach(itemchildren => { //遍历sku属性的具体信息
								if (skuinfo[attrtitle] == itemchildren
									.name) { //判断属性的title是否和后台传回的sku标题一致，一致加入对象
			...
			onBuyClicked(skudata={}) {
				...
				if (this.isBtn == 3) { //拼单
					let zfinfos = 0.00 + "," + 0.00 + "," + 0 + "," + 0.00 + "," + 0.00 + "," + 0.00 + "," + 0.00 +
						"," + 0 + "," + 0.00 + "," + 0;
					...
					this.$api.product.CommitPay_AD_PD({
						...
						if (res != '') {
							if (res.fsstate != 0) {
								...
								this.$router.push({
									path: '/groupswait?orderno=' + res.fsmes
								})
								return
							}
						}
...
<style scoped>
	...
	.buyer-protection-title i {
		font-size: 30px;
		vertical-align: middle;
		padding-right: 10px;
	}

	.buyer-protection-title span {
		vertical-align: middle;
		font-size: 16px;
	}
	...
	.buyer-protection-info p i {
		font-size: 22px;
		padding-right: 8px;
		vertical-align: middle;
	}

	.buyer-protection-info p span {
		vertical-align: middle;
	}

	.buyer-protection-info p em {
		font-style: normal;
		font-weight: bold;
	}
