		...
		<div class="deals-item" v-if="isdata">
			<div class="deals-item4" v-for="(item,index) in dealsList" :key="item.rid">
				<router-link :to="item | urlCompile">
					<div class="dealsImg">
						<img :style="{width:detailimg[index].isWidth ? detailimg[index].isWidth: '100%',height:detailimg[index].isHeight}" :index="index" @load="loadimg" :src="item.img" />
					</div>
		...
		mounted() {
			...
			if(this.dealsList.length>0){
				...
				for(let i=0;i<this.dealsList.length;i++){
					this.detailimg.push({'isWidth':'100%','isHeight':'auto'});
				}
			}
		},
		methods:{
...
<style scoped>
	.deals-product{margin:20px 5px 0}
	.deals-title{display: flex;}
	.deals-title h3{flex: 1;font-size:18px}
	.deals-title a{width:100px;color: #888;font-size: 14px;text-align: right;}
	.deals-item{width:calc(100% - 10px);display: flex; flex-wrap: wrap;background-color: #fff;border-radius: 5px;padding: 5px;margin-top: 10px;}
	.deals-item4{width: 25%;height: 150px;border: 1px solid #f7f7f7;box-sizing: border-box;}
	.deals-item4>a{display: flex; flex-direction: column;color: #000;}
	.dealsImg{width: 80px;height: 80px;margin:5px auto;}
	.deals-item4>a img{display: block;margin:0 auto;}
	.deals-item4>a span{font-size: 12px;line-height:20px;display: block;padding:0 8px;overflow: hidden;text-overflow: ellipsis; white-space: nowrap;}
	.oriprice{color:#e41c1d}
	.zkj{color:#888}
</style>
